...
var template = '/*! sink.%ext% build:' + package.version + ', %type%. Copyright(c) 2013 Eric Zhang, Michelle Bu, Rolland Wu MIT Licensed */'
  , prefix = '\n(function(exports){\n'
...
/**
 * Builds a custom sinkJS client distribution based on the transports that you
...
if (!module.parent){
  ...
  builder(args.length ? args : false, { minify:false }, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/sink.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the development build: sink.js');
  });
  ...
  builder(args.length ? args : false, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/sink.min.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the production build: sink.min.js');
  });
}
