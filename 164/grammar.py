...
class Production:
    '''
    LHS is the LHS symbol (redundancy beware: this symbol is also stored in the
    Rule)
    ...
    def toString (self,invRenamedTerminals):
        ...
        '''
        return self.LHS+" -> "+" ".join([invRenamedTerminals.get(sym,sym) for \
               sym in self.RHS])
...
class Grammar:
	...
	'''Return the rule for the ninterminal symbol, or None if symbol is not 
        defined'''
